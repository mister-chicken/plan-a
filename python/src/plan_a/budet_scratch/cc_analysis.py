...
def categorize_transaction(row: pd.Series) -> str:
    ...
    Categories: income, rent, utilities, dining_out, groceries, subway, taxi, shopping,
                subscriptions, home, banking, investment, other
    """
    ...
    # Handle banking-specific categories
    if category in ["income", "transfer_in"]:
        return "income"
    elif category == "credit_card_payment":
        return "credit_card_payment"
    elif category in ["investment", "trading", "robinhood"]:
        return "investment"
    elif category == "cash_withdrawal":
        return "cash_withdrawal"
    elif category == "payment_app":
        # Payment apps are often pass-through, skip or categorize as banking
        return "banking"
    elif category == "rent":
        return "rent"

    # Apollo Management - rental income
    apollo_keywords = ["apollo", "apollomanagement", "apollomanagemen"]
    if any(
        keyword in merchant.lower() or keyword in description.lower()
        for keyword in apollo_keywords
    ):
        # Apollo payments are always income (rent received)
        return "income"

    # ManageGo - rent payment
    managego_keywords = ["managego", "managegoprotectedm", "managegonorthflat"]
    if any(
        keyword in merchant.lower() or keyword in description.lower()
        for keyword in managego_keywords
    ):
        return "rent"
    ...
    if any(keyword in merchant or keyword in description for keyword in rent_keywords):
        # Distinguish between paying rent (expense) and receiving rent (income)
    ...
    if any(
        keyword in merchant or keyword in description for keyword in utility_keywords
    ):
        return "utilities"
    ...
    if any(
        keyword in merchant or keyword in description for keyword in subway_keywords
    ):
        return "subway"
    ...
    if any(
        keyword in merchant or keyword in description
    ...
    if any(
        keyword in merchant or keyword in description for keyword in shopping_keywords
    ):
        return "shopping"
    ...
    # Banking operations
    if source == "td_bank" or category in ["banking", "services"]:
        return "banking"
...
def get_monthly_spending_by_category(df: pd.DataFrame) -> pd.DataFrame:
    ...
    monthly = (
        expenses.groupby(["year_month", "analysis_category"])["amount"]
        .sum()
        .reset_index()
    )
    ...
    pivot = monthly.pivot(
        index="year_month", columns="analysis_category", values="amount"
    )
...
def get_category_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        expenses.groupby("analysis_category")["amount"]
        .agg(
            [
                ("total", "sum"),
                ("count", "count"),
...
def get_top_merchants_by_category(
    df: pd.DataFrame, category: str, n: int = 10
) -> pd.DataFrame:
...
def get_spending_trends(df: pd.DataFrame, categories: List[str] = None) -> pd.DataFrame:
    ...
    if categories:
        ...
        available_categories = [
            cat for cat in categories if cat in monthly_data.columns
        ]
...
def calculate_category_percentages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in monthly_data.columns:
        if col != "total":
            monthly_data[f"{col}_pct"] = (
                monthly_data[col] / monthly_data["total"] * 100
            ).round(2)
